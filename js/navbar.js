...
document.addEventListener('DOMContentLoaded', function() {
  const comoFuncionaLink = document.querySelector('.nav-link[href="#home"]');
  if (comoFuncionaLink) {
    comoFuncionaLink.addEventListener('click', function(e) {
      e.preventDefault();
      const target = document.getElementById('home');
      if (target) {
        const targetY = target.getBoundingClientRect().top + window.pageYOffset - 60;
        smoothScrollTo(targetY, 800);
      }
    });
  }

  const vantagensLink = document.querySelector('.nav-link[href="#menu"]');
  if (vantagensLink) {
    vantagensLink.addEventListener('click', function(e) {
      e.preventDefault();
